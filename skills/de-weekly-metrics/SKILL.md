### 21. TRUE Anomalies with Mix and Contribution (8-Week Baseline)
```sql
...
description: "Weekly Data Engineering metrics with full category deep dive plus Dynamic Tables, Snowpark, and ingestion focus. Triggers: DE weekly, data engineering metrics, dynamic tables weekly, snowpark metrics, DT analysis, ingestion metrics."
---
...
### Step 5: Dynamic Tables Deep Dive
...
### Step 8: Anomalous Customer Analysis
...
2. Use Case breakdown with target variance, Y/Y growth, mix %, and outsized mover flags
3. Feature tables with concentration, customer movers, Y/Y growth, mix %, and outsized mover flags
4. Dynamic Tables section (Revenue + Count + Storage + Y/Y)
5. Snowpark section (DE + All-Up + Y/Y)
6. TB Ingested section with top customers per modality
7. **Anomalous Customers section** with AE/SE, SE comments, use case descriptions, and feature details
8. Key Signals summary
...
### 9. Dynamic Table Count (last day of week)
```sql
WITH all_tables AS (
    SELECT 
        c._date AS date,
...
### 10. Dynamic Table Storage (TB, last day of week)
```sql
SELECT 
    a.ds,
...
### 12. TB Ingested by Modality (with Y/Y)
```sql
WITH current_week AS (
...
### 13. Top Customers per Ingestion Modality
```sql
...
### 17. Customer-Feature Mix vs Contribution (Outsized Mover Detection)
```sql
...
### 18. Historical Consumption for Customer Anomaly Analysis
```sql
...
### 18. Historical TB Ingested for Modality Anomaly Analysis
```sql
...
### 20. Use Case Details from dim_use_case (SE Comments & Descriptions)
```sql
...
- ✋ Step 9: Before finalizing report
...
- Key signals and risks including outsized movers
